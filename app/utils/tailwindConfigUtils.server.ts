...
async function getTailwindPackageJsonPath() {
	try {
		const tailwindPackageJsonPath = import.meta.resolve(
			'tailwindcss/package.json',
			`file://${process.cwd()}/`,
		)
		console.log(tailwindPackageJsonPath)
	} catch (error) {
		console.error('Error resolving tailwindcss package path:', error)
	}
}
...
const tailwindVersion = import.meta.resolve(
	'tailwindcss/package.json',
	`file://${process.cwd()}/`,
)
...
const flattenColorPalettePath = require.resolve(
...
const resolveConfigToJson = async (configPath: string) => {
...
const transformConfig = config => {
	...
	config.theme.backgroundColor = flattenColorPalette(
...
const replaceWithOverrides = theme => {
	if (theme.configViewer && theme.configViewer.themeReplacements) {
		Object.keys(theme.configViewer.themeReplacements).forEach(key => {
			theme = findAndReplaceRecursively(
				theme,
				key,
